...
fonts <- list(
  sans = list(
    plain = "C:/Users/jchoe/AppData/Local/Microsoft/Windows/Fonts/OpenSans-Regular.ttf",
    italic = "C:/Users/jchoe/AppData/Local/Microsoft/Windows/Fonts/OpenSans-Italic.ttf"
  ),
  serif = list(
    plain = "C:/Users/jchoe/AppData/Local/Microsoft/Windows/Fonts/RobotoSlab-Regular.ttf"
  )
)

knitr::opts_chunk$set(fig.width = 6, message = FALSE, warning = FALSE, comment = " ", cache = FALSE, dev = "svglite", dev.args = list(user_fonts = fonts))
...
```{r, echo=FALSE, out.width="650px",out.height="400px", fig.align="center", fig.cap="<a href='https://xkcd.com/1945/'>https://xkcd.com/1945</a>"}
...
- Walk through implementations of these ideas (w/ [R code](https://raw.githubusercontent.com/yjunechoe/yjunechoe.github.io/master/flipbooks/tsg_lab_ggplot2_2020/tsg_lab_ggplot2.Rmd))
...
```{r text, echo = FALSE, eval = FALSE, dev = "png", dpi = 150}
state_election_plot +
  ...
  theme(text = element_text(family = "Roboto")) + #ROTATE
...
`r chunk_reveal("text", omit = "#OMIT", break_type = "rotate", widths = c(40, 60), chunk_options = "fig.height = 7, dev = 'png', dpi = 150, dev.args = list(type = 'cairo-png')")`
...
<video controls src="img/HCL_space.webm">
]
...
<h1 style='margin-bottom:0px'>The curse of the rainbow</h1>

```{r, out.height="450px", out.width="450px", fig.align='center'}
colorspace::specplot(grDevices::rainbow(7))
```
...
```{r, fig.show='hide'}
corr_left <- ggcorrplot::ggcorrplot(cor(mtcars)) + scale_fill_gradient2(low = "blue", mid = "white", high = "red", limits = c(-1, 1))
corr_right <- ggcorrplot::ggcorrplot(cor(mtcars)) + scale_fill_continuous_diverging(palette = "Blue-Red 3", limits = c(-1, 1))
...
```{r, echo = FALSE}
bernie <- as.raster(magick::image_read("https://raw.githubusercontent.com/nicksawhney/bernie-sits/main/bernie.png"), interpolate = TRUE)
```


```{r themes, echo = FALSE, eval = FALSE}
state_election_plot_C$layers <- NULL
state_election_plot_C +
  theme(panel.background = element_rect(fill = NA)) +
  theme(legend.background = element_rect(fill = NA)) +
  theme(axis.line = element_line()) +
  ...
  list(scale_color_manual(values = c("#65657F", "#855F4C", "#556760")), annotation_raster(bernie, xmin = 1990, xmax = 2016, ymin = .4, ymax = .65)) + #ROTATE
...
`r chunk_reveal("colors2", break_type = "rotate", widths = c(40, 60))`
...
```{r, echo = FALSE, out.width = "400px"}
state_election_plot_C + geom_line()
```
...
knitr::include_graphics("tsg_lab_ggplot2_files/final.gif")
```
...
```{r}
object.size(state_election_plot_D)
...
```{r vowels2, echo = FALSE, eval=FALSE, dev="png", dpi = 150}
sim_vowel_data_all %>% 
  group_by(Vowel) %>% 
...
`r chunk_reveal("vowels2", break_type = "auto", widths = c(40, 60), chunk_options = "fig.height = 7, dev = 'png', dpi = 150, dev.args = list(type = 'cairo-png')")`
...
library(cowplot)
ggdraw(p) + 
  draw_label("Accuracy", x = .125, y = .9, fontfamily = "Open Sans", size = 14, color = "#2D2D2D") 
```
